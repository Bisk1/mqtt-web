...
public class ClientState {
	 ...
	 * QoS 0 message are not good enough for checking a session is
	 * alive as they are one way messages.
	 * 
	 * If a ping has been sent but no data has been received in the 
	 * last keepalive interval then the session is deamed to be broken.
	...
	public MqttToken checkForActivity(IMqttActionListener pingCallback) throws MqttException {
		...
		if (connected && this.keepAlive > 0) {
            ...
            synchronized (pingOutstandingLock) {
                ...
                // Is the broker session lost because the broker did not reply to my ping?
                if (pingOutstanding > 0 && (time - lastInboundActivity >= keepAlive + delta)) {
                	...
                	// A ping is outstanding but no packet has been received in KA so session is deemed broken
                ...
                // Is the broker session lost because I could not get any successful write for 2 keepAlive intervals?
                ...
                //    In this case only one ping is sent. If not confirmed, client will assume a lost session to the broker.
                ...
                //    If lacking to do so, the broker will consider my session lost and cut my socket.
                if ((pingOutstanding == 0 && (time - lastInboundActivity >= keepAlive - delta)) ||
