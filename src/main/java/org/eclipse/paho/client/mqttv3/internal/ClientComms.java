...
public class ClientComms {
	...
	private Object	conLock = new Object();  	// Used to synchronize session state
	 ...
	 * network session, receive thread, and keep alive thread.
	 * @param options The {@link MqttConnectOptions} for the session
	 * @param token The {@link MqttToken} to track the session
	 * @throws MqttException if an error occurs when connecting
	 */
	public void connect(MqttConnectOptions options, MqttToken token, MqttMessageSentListener messageListener) throws MqttException {
		...
		synchronized (conLock) {
			if (isDisconnected() && !closePending) {
				...
				if (messageListener != null) {
					messageListener.messageSent(connect);
				}
			}
			else {
	...
	/**
	 * Shuts down the session to the server.
	 ...
	 * @param token the {@link MqttToken} To track closing the session
	 * @param reason the {@link MqttException} thrown requiring the session to be shut down.
	 */
	...
	/**
	 * Disconnect the session and reset all the states.
