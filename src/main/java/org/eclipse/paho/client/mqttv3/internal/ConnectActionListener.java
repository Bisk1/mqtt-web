...
/**
 * <p>This class handles the session of the AsyncClient to one of the available URLs.</p>
 ...
 * <p>An attempt is made to connect to each URL in the list until either a session attempt succeeds or all the URLs have been tried</p>
 * <p>If a session succeeds then the users token is notified and the users onSuccess callback is called.</p>
 * <p>If a session fails then another URL in the list is attempted, otherwise the users token is notified
...
/**
   ...
   * 
   * @param token the {@link IMqttToken} from the successful session
   */
  public void onSuccess(IMqttToken token) {
    ...
    if(mqttCallbackExtended != null){
    	String serverURI = comms.getNetworkModule().getServerURI();
   ...
   * 
   * @param token the {@link IMqttToken} from the failed session attempt
   * @param exception the {@link Throwable} exception from the failed session attempt
   */
   ...
   */
  public void connect(MqttMessageSentListener mqttMessageSentListener) throws MqttPersistenceException {
    ...
    try {
      comms.connect(options, token, mqttMessageSentListener);
    }
   ...
   * @param mqttCallbackExtended the {@link MqttCallbackExtended} to be called when the session completes
   */
