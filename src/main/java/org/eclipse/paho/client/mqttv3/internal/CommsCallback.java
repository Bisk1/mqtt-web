...
public class CommsCallback implements Runnable {
				...
				if (!Thread.currentThread().equals(callbackThread)) {
					try {
						synchronized (workAvailable) {
							// @TRACE 701=notify workAvailable and wait for createOrReplaceMqttSession
							// to finish
	...
	public void run() {
		final String methodName = "createOrReplaceMqttSession";
	...
	/**
	 * This method is called when the session to the server is lost. If there
	 * is no cause then it was a clean disconnect. The connectionLost callback
	 * will be invoked if registered and createOrReplaceMqttSession on the thread that requested
	 ...
	 * 
	 * @param cause  the reason behind the loss of session.
	 */
	public void connectionLost(MqttException cause) {
		...
		// the session lost listener of the problem.
		try {
			...
			// Just log the fact that a throwable has caught session lost
