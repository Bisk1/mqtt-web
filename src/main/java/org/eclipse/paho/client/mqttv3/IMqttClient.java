...
public void subscribe(String topicFilter, int qos, IMqttMessageListener messageListener) throws MqttException;
  ...
  public void unsubscribe(String[] topicFilters) throws MqttException;
	 ...
	 * once the message has been delivered. In the event the session fails or the client
	 * stops, any messages that are in the process of being delivered will be delivered once
	 * a session is re-established to the server on condition that:</p>
	 * <ul>
	 * <li>The session is re-established with the same clientID</li>
	 ...
	 * set to false</li>
	 * </ul>
	 * <p>In the event that the session breaks or the client stops it is still possible to determine
	 * when the delivery of the message completes. Prior to re-establishing the session to the server:</p>
	 * <ul>
