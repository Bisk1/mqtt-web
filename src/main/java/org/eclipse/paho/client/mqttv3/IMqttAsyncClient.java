...
public interface IMqttAsyncClient {
	 ...
	 * <p>The session will be established using the options specified in the
	 * {@link MqttConnectOptions} parameter.
	 * </p>
	 *
	 * @param options a set of session parameters that override the defaults.
	 * @throws MqttSecurityException  for security related problems
	 * @throws MqttException  for non security related problems
	 * @return token used to track and wait for the connect to complete. The token
	 ...
	 */
	public IMqttToken connect(MqttConnectOptions options, Object userContext, MqttMessageSentListener mqttMessageSentListener, IMqttActionListener callback) throws MqttException, MqttSecurityException;
	 ...
	 * next time a session is made QoS 1 and 2 messages that
	 ...
	 * <p>
	 * Because the client is able to establish the TCP/IP session to a none MQTT server and it will certainly fail to
	 ...
	 * In the event the session fails or the client stops. Messages will be delivered to the
	 * requested quality of service once the session is re-established to the server on condition that:
	 * </p>
	 * <ul>
	 * <li>The session is re-established with the same clientID
	 * <li>The original session was made with (@link MqttConnectOptions#setCleanSession(boolean)}
	 * set to false
	 * <li>The session is re-established with (@link MqttConnectOptions#setCleanSession(boolean)}
	 * set to false
	 ...
	 */
	public IMqttToken subscribe(String[] topicFilters, int[] qos, Object userContext, MqttMessageSentListener mqttMessageSentListener, IMqttActionListener callback)
			throws MqttException;
	 ...
	 * @throws MqttException if there was an error registering the subscription.
	 */
	public IMqttToken subscribe(String topicFilter, int qos, Object userContext, IMqttActionListener callback, MqttMessageSentListener mqttMessageSentListener, IMqttMessageListener messageListener) throws MqttException;
	 ...
	 * <li>The session to the server has been lost</li>
