 ...
 * that allow an operation to createOrReplaceMqttSessionAndConnect in the background.
 *
 * <p>
...
public class MqttAsyncClient implements IMqttAsyncClient {
	 ...
	 * using the {@link MqttConnectOptions#setServerURI(String)
	 * setServerURI} method on MqttConnectOptions.
	 *
	 * <p>
	 ...
	 * of session are supported <code>tcp://</code> for a TCP session and
	 * <code>ssl://</code> for a TCP session secured by SSL/TLS. For example:
	 * </p>
	 * <ul>
	 ...
	 *            {@link MqttConnectOptions#setServerURI(String)}
	 * @param clientId
...
//				}
	...
	public IMqttToken connect(Object userContext, IMqttActionListener callback)
		...
		return this.connect(new MqttConnectOptions(), userContext, null, callback);
	}
	...
	public IMqttToken connect(MqttConnectOptions options) throws MqttException {
		return this.connect(options, null, null, null);
	}
	 ...
	 * org.eclipse.paho.client.mqttv3.IMqttActionListener)
	 */
	public IMqttToken connect(MqttConnectOptions options, Object userContext, MqttMessageSentListener mqttMessageSentListener,
		...
		log.fine(CLASS_NAME, methodName, "103",
				new Object[] {options.isCleanSession(), options.getConnectionTimeout(),
						options.getKeepAliveInterval(), options.getUserName(),
		...
		connectActionListener.connect(mqttMessageSentListener);
	 ...
	 * <p>
	 * Because the client is able to establish the TCP/IP session to a none
	...
	public String getCurrentServerURI() {
		return comms.getNetworkModule().getServerURI();
	}
	 ...
	 * sends PingReq to server to keep the session to server. For some
	...
	public IMqttToken subscribe(String topicFilter, int qos, Object userContext, MqttMessageSentListener mqttMessageSentListener, IMqttActionListener callback)
			throws MqttException {
		return this.subscribe(new String[] { topicFilter }, new int[] { qos }, userContext, mqttMessageSentListener, callback);
	}

    public IMqttToken subscribe(String[] topicFilters, int[] qos, Object userContext,
                                MqttMessageSentListener mqttMessageSentListener, IMqttActionListener callback)
			throws MqttException {
		...
		mqttMessageSentListener.messageSent(register);
	...
	public IMqttToken subscribe(String topicFilter, int qos, Object userContext, IMqttActionListener callback,
                                MqttMessageSentListener mqttMessageSentListener, IMqttMessageListener messageListener) throws MqttException {

		return this.subscribe(new String[] { topicFilter }, new int[] { qos }, userContext, callback, null,
	...
	public IMqttToken subscribe(String topicFilter, int qos, IMqttMessageListener messageListener)
			throws MqttException {
		return this.subscribe(new String[] { topicFilter }, new int[] { qos }, null, null,null,
                new IMqttMessageListener[] { messageListener });
	}

	public IMqttToken subscribe(String[] topicFilters, int[] qos, IMqttMessageListener[] messageListeners)
			throws MqttException {
		return this.subscribe(topicFilters, qos, null, null, null, messageListeners);
	}

    public IMqttToken subscribe(String[] topicFilters, int[] qos, Object userContext, IMqttActionListener callback,
			MqttMessageSentListener mqttMessageSentListener, IMqttMessageListener[] messageListeners) throws MqttException {

		if ((messageListeners.length != qos.length) || (qos.length != topicFilters.length)) {
			throw new IllegalArgumentException();
		}

		IMqttToken token = this.subscribe(topicFilters, qos, userContext, mqttMessageSentListener, callback);
	...
	private void attemptReconnect() {
		...
		log.fine(CLASS_NAME, methodName, "500", new Object[] { this.clientId });
		try {
			connect(this.connOpts, this.userContext, null, new MqttReconnectActionListener(methodName));
	...
	private class ReconnectTask extends TimerTask {
		private static final String methodName = "ReconnectTask.createOrReplaceMqttSessionAndConnect";
