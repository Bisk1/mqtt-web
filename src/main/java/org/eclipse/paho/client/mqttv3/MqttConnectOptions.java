...
public class MqttConnectOptions {
	...
	/**
	 * The default session timeout in seconds if one is not specified
	 */
	...
	private int MqttVersion = MQTT_VERSION_3_1_1;
	 ...
	 * <li>The session timeout period is 30 seconds</li>
	 ...
	 * @return the password to use for the session.
	 */
	...
	/**
	 * Sets the password to use for the session.
	...
	/**
	 * Returns the user name to use for the session.
	 * @return the user name to use for the session.
	 */
	...
	/**
	 * Sets the user name to use for the session.
	...
	/**
	 * Sets the "Last Will and Testament" (LWT) for the session.
	 * In the event that this client unexpectedly loses its session to the
	 ...
	 * This value, measured in seconds, defines the maximum time interval
	 * the client will wait for the network session to the MQTT server to be established.
	 * The default timeout is 30 seconds.
	 * A value of 0 disables timeout processing meaning the client will wait until the
	 * network session is made successfully or fails.
	 ...
	 * using an SSL session, an <code>SSLSocketFactory</code> can be used
	...
	/**
	 * Returns the SSL properties for the session.
	 * @return the properties for the SSL session
	 */
	...
	/**
	 * Sets the SSL properties for the session.
	 * <p>Note that these
	...
	/**
     * Returns the HostnameVerifier for the SSL session.
     * @return the HostnameVerifier for the SSL session
     */
    ...
    /**
     * Sets the HostnameVerifier for the SSL session. Note that it will be
     * used after handshake on a session and you should do actions by
	 ...
	 * restarts of the client, the server and the session. As state is maintained:
	 * <ul>
	 * <li>Message delivery will be reliable meeting
	 * the specified QOS even if the client, server or session are restarted.
	 * <li> The server will treat a subscription as durable.
	 * </ul>
	 * <li>If set to true the client and server will not maintain state across
	 * restarts of the client, the server or the session. This means
	 * <ul>
	 * <li>Message delivery to the specified QOS cannot be maintained if the
	 * client, server or session are restarted
	...
	/**
	 * Return a list of serverURI the client may connect to
	 * @return the serverURI or null if not set
	 */
	...
	/**
	 * Set a list of one or more serverURI the client may connect to.
	 * <p>
	 * Each <code>serverURI</code> specifies the address of a server that the client may
	 * connect to. Two types of
	 * session are supported <code>tcp://</code> for a TCP session and
	 * <code>ssl://</code> for a TCP session secured by SSL/TLS.
	 * For example:
	 * <ul>
	 ...
	 * <p>
	 * If serverURI is set then it overrides the serverURI parameter passed in on the
	 ...
	 * the list until a session is established with a server. If a session cannot be made to
	...
	public void setMqttVersion(int MqttVersion)throws IllegalArgumentException {
		if (MqttVersion != MQTT_VERSION_3_1 &&
	 ...
	 *  reconnect to the server in the event that the session is lost.</li>
	 *  <li>If set to true, in the event that the session is lost, the client
